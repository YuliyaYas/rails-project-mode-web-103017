class EventsController < ApplicationController
  ...
  def create
      ...
      if @event.save
        flash[:success] = "Tell you right now...if there's no wine we not going!"
        redirect_to event_path(@event)
      else
        flash[:danger] = "Sike! Try Again!"
      end
    end
  ...
  def user_attend
      ...
      flash[:success] = "#{@event.name} has been added to your events...It's Lit!"
  ...
  def authorized
      ...
      if @event.user_id == @user.id
        ...
        flash[:danger] = "This Ain't Yours SON!"
        redirect_to event_path(@event)
      end
    end
